...
```{r}
# Create scatter plot of CO2 Emission vs. Mortality Rate. Remove outlier (Texas).
rateEmissionsData <- blogData %>%
  filter(CO2_Emission != 711.0)

rateEmissions <- ggplot(blogData, aes(x = CO2_Emission, y = Rate)) +
  geom_point() + 
  xlab("CO2 Emission") +
  ylab("Mortality Rate") +
  geom_smooth() 
